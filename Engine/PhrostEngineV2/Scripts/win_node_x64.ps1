...
# --- Node.js Paths (x64) ---
# Matches the folder name created in your GitHub Action: 'node-x64'
$env:NODE_ROOT = (Join-Path $SCRIPT_DIR "..\..\deps\node-x64" | Resolve-Path)
$NODE_BIN = Join-Path $env:NODE_ROOT "node.exe"
$NPM_BIN = Join-Path $env:NODE_ROOT "npm.cmd"
...
# --- Build ---
Write-Host "### 1. Starting Swift Build (Node x64 Target)... ###"

# Using x86_64 triple and standard flags (No PHP defines)
...
# UPDATED: Pointing to the x86_64 build artifact folder
...
try {
    # Using the x64 paths defined at top of script
...
# Copy JS files (assuming they are in Runtime/javascript based on your structure)
